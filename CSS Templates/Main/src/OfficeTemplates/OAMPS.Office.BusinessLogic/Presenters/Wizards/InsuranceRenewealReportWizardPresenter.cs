    ...
    public class InsuranceRenewealReportWizardPresenter : BaseWizardPresenter
    {
            ...
            for (var index = fragements.Count - 1; index >= 0; index--)
            {
                ...
                if (Document.MoveCursorToStartOfBookmark(bookmarkTemplate))
                {
                    ...
                    Document.RenameTable(Constants.WordTables.Liabilityrisksandexposures,
                        Constants.WordTables.Liabilityrisksandexposures + f.Id + f.Order);
                    Document.RenameTable(Constants.WordTables.AssetRiskProtection,
                        Constants.WordTables.AssetRiskProtection + f.Id + f.Order);
                    Document.RenameTable(Constants.WordTables.IncomeandotherFinancialExposures,
                        Constants.WordTables.IncomeandotherFinancialExposures + f.Id + f.Order);
                    Document.RenameTable(Constants.WordTables.PolicyWording,
                        Constants.WordTables.PolicyWording + f.Id + f.Order);


                    Document.RenameBookmark(Constants.WordBookmarks.SummaryOfCoverStart,
                        Constants.WordBookmarks.SummaryOfCoverStart + f.Id + f.Order);
                    Document.RenameBookmark(Constants.WordBookmarks.SummaryOfCoverEnd,
                        Constants.WordBookmarks.SummaryOfCoverEnd + f.Id + f.Order);
                }
                c++;
            }
        ...
        public void SendUFIMessage(InsuranceRenewalReport template, string recipients)
        {
            ...
            var bodywithValues =
                $@"Hello, 
                                ...
                                <br/><br/> {
                    name
                    } has just created an Insurance Renewal Report recommending an unauthorised foreign insurer.
        ...
        public IInsuranceRenewalReport LoadIncludedPolicyClasses(IInsuranceRenewalReport template)
        {
                ...
                //foreach (var i in split)
            {
                ...
                if (d.Length == 3)
                {
                    var id = (d.Length == 0) ? string.Empty : d[0].ToString(CultureInfo.InvariantCulture);
                    var order = int.Parse(d[2].Substring(d[2].IndexOf("r", StringComparison.Ordinal) + 1));
                    var p = new PolicyClass
                    {
                        Title =
                            Document.ReadContentControlValue(id + order +
                                                             Constants.WordContentControls.ClassOfInsuranceTitle),
                        Id = id,
