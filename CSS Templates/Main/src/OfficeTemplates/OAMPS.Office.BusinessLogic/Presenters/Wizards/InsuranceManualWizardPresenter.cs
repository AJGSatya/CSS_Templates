    ...
    public class InsuranceManualWizardPresenter : BaseWizardPresenter
    {
        ...
        public void PopulateProgramSummarys(List<IPolicyClass> policies, string fragmentUrl)
        {
            ...
            for (var i = policies.Count - 1; i > -1; i--)
            {
                ...
                if (Document.HasBookmark("coInsuranceText"))
                {
                    ...
                    Document.RenameBookmark("coInsuranceText", "coInsuranceText" + policy.Id + policy.Order);
                    ...
                    {
                        if (Document.MoveCursorToStartOfBookmark("coInsuranceText" + policy.Id + policy.Order))
                        {
                            Document.TypeText(
                ...
                Document.RenameControl("ClassOfInsuranceTitle", "ClassOfInsuranceTitle" + policy.Id + policy.Order);
                Document.RenameControl("Insurer", "Insurer" + policy.Id + policy.Order);
                Document.RenameControl("PolicyNumber", "PolicyNumber" + policy.Id + policy.Order);

                Document.PopulateControl("ClassOfInsuranceTitle" + policy.Id + policy.Order, policy.Title);
                Document.PopulateControl("PolicyNumber" + policy.Id + policy.Order, policy.PolicyNumber);
                Document.PopulateControl("Insurer" + policy.Id + policy.Order, GenerateInsurers(policy.CurrentInsurer));
                    //dev comment: this is actually the recommended insurer.

                Document.RenameTable(Constants.WordTables.PolicyWording,
                    Constants.WordTables.PolicyWording + policy.Id + policy.Order);
                Document.RenameTable(Constants.WordTables.Liabilityrisksandexposures,
                    Constants.WordTables.Liabilityrisksandexposures + policy.Id + policy.Order);
                Document.RenameTable(Constants.WordTables.AssetRiskProtection,
                    Constants.WordTables.AssetRiskProtection + policy.Id + policy.Order);
                Document.RenameTable(Constants.WordTables.IncomeandotherFinancialExposures,
                    Constants.WordTables.IncomeandotherFinancialExposures + policy.Id + policy.Order);

                Document.RenameBookmark(Constants.WordBookmarks.SummaryOfCoverStart,
                    Constants.WordBookmarks.SummaryOfCoverStart + policy.Id + policy.Order);
                Document.RenameBookmark(Constants.WordBookmarks.SummaryOfCoverEnd,
                    Constants.WordBookmarks.SummaryOfCoverEnd + policy.Id + policy.Order);
        ...
        public void PopulatePolicyTable(List<IPolicyClass> policies)
        {
            ...
            foreach (var policy in policies)
            {
                ...
                Document.PopulateTableCell(r, 2, GenerateInsurers(policy.CurrentInsurer),
                    Constants.WordTables.InsuranceManualProgramSummary);
