...
using Enums = OAMPS.Office.BusinessLogic.Helpers.Enums;

namespace OAMPS.Office.Word.Views.Wizards
{
    public partial class ShortFormProposalWizard : BaseWizardForm
    {
        protected static List<ISharePointListItem> DocumentTitles;
        ...
        private readonly ShortFormPropsalWizardPresenter _wizardPresenter;
        private string _defaultPath = "G:\\";
        ...
        public ShortFormProposalWizard(OfficeDocument document, Helpers.Enums.FormLoadType loadType)
        {
            InitializeComponent();
            ...
            rdoSegment1.Validating += rdoSegment_Validating;
            ...
            _wizardPresenter = new ShortFormPropsalWizardPresenter(document, this);
            BaseWizardPresenter = _wizardPresenter;
            LoadType = loadType;

            tvaPolicies.Resize += tvaPolicies_Resize;

            //ShouldUpdateTemplate(Settings.Default.TemplateLibraryName, "Insurance proposal - short form.docx");
        }
        ...
        private void tvaPolicies_Resize(object sender, EventArgs e)
        {
            ...
            tvcPolicy.Width = (tvaPolicies.Width/5) + 150;
        }
        ...
        {
            if (rdoSegment1.Checked == false && rdoSegment2.Checked == false && rdoSegment3.Checked == false &&
                rdoSegment4.Checked == false &&
        ...
        private void InsuranceRenewalReport_Load(object sender, EventArgs e)
        {
            ...
            {
                //RegenerateFormLoad();
                //lblLogoTitle.Text = @"Arthur J. Gallagher";
                //lblCoverPageTitle.Text = @"Dark Blue";
        ...
        private void LoadForm()
        {
            ...
            {
                MessageBox.Show(@"You cannot make any changes to this template through the wizard",
            ...
            else //new template
            {
                Task.Factory.StartNew(() =>
                {
                    ...
                    LoadDocumentTitles(uiScheduler);
                });
            }
        }

        private void LoadDocumentTitles(TaskScheduler uiScheduler)
        {
            if (Cache.Contains(Constants.CacheNames.ShortFormProposalDocumentTitles))
            {
                DocumentTitles =
                    ((List<ISharePointListItem>) Cache.Get(Constants.CacheNames.ShortFormProposalDocumentTitles));
            }
            else
            {
                DocumentTitles = GetDocumentTitles();
                Cache.Add(Constants.CacheNames.ShortFormProposalDocumentTitles, DocumentTitles, new CacheItemPolicy());
            }

            if (uiScheduler == null)
                BindDocumentTitlesControl(DocumentTitles);
            else
                Task.Factory.StartNew(() => BindDocumentTitlesControl(DocumentTitles), CancellationToken.None,
                    TaskCreationOptions.None, uiScheduler);
        }

        protected virtual List<ISharePointListItem> GetDocumentTitles()
        {
            var list = ListFactory.Create(Settings.Default.ListProposalShortDocumentTitles, true);
            var presenter = new SharePointListPresenter(list, this);
            return presenter.GetItems();
        }

        private void BindDocumentTitlesControl(List<ISharePointListItem> items)
        {
            cbDocumentTitle.DataSource = items;
            cbDocumentTitle.DisplayMember = "Title";
            cbDocumentTitle.ValueMember = "Title";
        }
        ...
        private void ReloadPolicyClasses(IInsuranceRenewalReport v, bool reload)
        {
            ...
            for (var index = v.SelectedDocumentFragments.Count - 1; index >= 0; index--)
            {
        ...
        {
            _storedStatutory =
                _wizardPresenter.ReadDocumentProperty(Constants.WordDocumentProperties.StatutoryInformation);
        ...
        private InsuranceRenewalReport GenerateTempalteObject()
        {
            ...
            var template = new InsuranceRenewalReport
            {
                DocumentTitle = cbDocumentTitle.Text,
                //BaseWizardPresenter.ReadDocumentProperty("Title"), //Constants.TemplateNames.InsuranceRenewalReport,
        ...
        private void PopulateDocument()
        {
            ...
            _wizardPresenter.PopulateBasisOfCover(_selectedDocumentFragments,
                Settings.Default.InsuranceProposalShortProgramSummary);
            ...
            // var image = GetServicePlanImage(SelectedSegment);

            var servicePlanToUse = GetServicePlanImageDoument(SelectedSegment);
            _wizardPresenter.PopulateSegmentServicePlanImage(servicePlanToUse);
            ...
            if (rdoClitProfileYes.Checked)
                _wizardPresenter.PopulateclientProfile(Settings.Default.FragmentClientProfileShortFormProposal);


            var urls = (from object i in lbProfiles.Items select ((string) i)).ToList();
            _wizardPresenter.PopulateServiceTeam(urls);
            ...
            {
                TopMost = false;
                    //this is important, as there is a chance that outlook will prompt the users and if our wizard is forced to the top and outlook is minimised they will not see the outlook dialog and processing will hang.
                Task.Factory.StartNew(() => SendUFI(template))
                    .ContinueWith((task => { if (task.IsFaulted) OnError(task.Exception); }));
            }

            if (SelectedSegment == Enums.Segment.Four || SelectedSegment == Enums.Segment.Five)
            {
                TopMost = false;
                    //this is important, as there is a chance that outlook will prompt the users and if our wizard is forced to the top and outlook is minimised they will not see the outlook dialog and processing will hang.
                Task.Factory.StartNew(() => SendSeg4or5Email(template))
                    .ContinueWith((task => { if (task.IsFaulted) OnError(task.Exception); }));
            }
        }

        //private string GetServicePlanImage(Enums.Segment segment)
        //{
        //    var contextUrl = Settings.Default.SharePointContextUrl;
        //    var listTitle = "Segment service plan images"; //todo: move to config

        //    var sharePointPictureLibry = new SharePointPictureLibrary(contextUrl, listTitle, false,
        //        string.Format(Constants.SharePointQueries.GetItemByTitleQuery, segment));
        //    var presenter = new SharePointPictureLibraryPresenter(sharePointPictureLibry, this);
        //    const string cacheName = Constants.CacheNames.ServicePlanImages;
        ...
        //        //  Cache.Add(cacheName, th, new CacheItemPolicy());  //dont wanna cache, no time
        //    }

        //    return th.Count > 0 ? th[0].FullImageUrl : string.Empty;
        //}

        private string GetServicePlanImageDoument(Enums.Segment segment)
        {
            if (segment == Enums.Segment.One)
                return Settings.Default.Segment1Graphic;
            if (segment == Enums.Segment.Two)
                return Settings.Default.Segment2Graphic;
            if (segment == Enums.Segment.Three)
                return Settings.Default.Segment3Graphic;
            if (segment == Enums.Segment.Four)
                return Settings.Default.Segment4Graphic;
            if (segment == Enums.Segment.Five)
                return Settings.Default.Segment5Graphic;
            return string.Empty;
        }
        ...
        private void SendSeg4or5Email(InsuranceRenewalReport template)
        {
            var list = ListFactory.Create("Configuration");
            var presenter = new SharePointListPresenter(list, null);

            var autoSendItem = presenter.GetItemByTitle("SFProposal.AutoSendEmail");
            var shouldAutoSend = autoSendItem.GetFieldValue("Value");

            if (string.Equals(shouldAutoSend, "no", StringComparison.OrdinalIgnoreCase))
                return;

            var item = presenter.GetItemByTitle("SFProposal.Recipients");
            var address = item.GetFieldValue("Value");
            if (string.IsNullOrEmpty(address)) return;
            _wizardPresenter.SendSeg4Or5Message(template, address);
        }
        ...
        private void rdoFeeOnly_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload &&
                string.Equals(_storedRemuneration, Enums.Remuneration.Fee.ToString(), StringComparison.OrdinalIgnoreCase))
            {
        ...
        private void rdoCombination_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload &&
                string.Equals(_storedRemuneration, Enums.Remuneration.Combined.ToString(),
                    StringComparison.OrdinalIgnoreCase))
            {
        ...
        private void rdoRetailFSG_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload &&
                string.Equals(_storedStatutory, Enums.Statutory.Retail.ToString(), StringComparison.OrdinalIgnoreCase))
            {
        ...
        private void rdoWholesaleWithRetail_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload &&
                string.Equals(_storedStatutory, Enums.Statutory.WholesaleWithRetail.ToString(),
                    StringComparison.OrdinalIgnoreCase))
            {
        ...
        private void rdoWholesale_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload &&
                string.Equals(_storedStatutory, Enums.Statutory.Wholesale.ToString(), StringComparison.OrdinalIgnoreCase))
            {
        ...
        private void btnAssistantAccountExecutiveLookup_Click_1(object sender, EventArgs e)
        {
            var opdProfileDialog = new OpenFileDialog
            {
                InitialDirectory = _defaultPath,
                Filter = @"profiles (*.docx)|*.docx",
                FilterIndex = 1,
                RestoreDirectory = true
            };

            if (opdProfileDialog.ShowDialog() != DialogResult.OK) return;

            lbProfiles.Items.Add(opdProfileDialog.FileName);
            _defaultPath = Path.GetDirectoryName(opdProfileDialog.FileName);
        }
        ...
        private void rdoSegment1_CheckedChanged(object sender, EventArgs e)
        {
            SelectedSegment = Enums.Segment.One;

            if (Reload &&
                string.Equals(_storedSegment, Enums.Segment.One.ToString(), StringComparison.OrdinalIgnoreCase))
            {
                if (rdoSegment1.Checked == false)
                {
                    if (ContinueWithSignificantChange(sender, true)) GenerateNewTemplate = true;
                }
            }
