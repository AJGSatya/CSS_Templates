...
namespace OAMPS.Office.Word.Views.Wizards
{
    ...
    {
        protected static List<IQuestionClass> Questions;
        private readonly IDocument _document;
        private readonly List<IPolicyClass> _selectedQuestionnaireFragments = new List<IPolicyClass>();
        ...
        public QuoteSlipWizard(OfficeDocument document, Enums.FormLoadType loadType)
        {
            ...
            clbQuestions.Validating += clbQuestions_Validating;
            txtClientName.Validating += txtClientName_Validating;

            //ShouldUpdateTemplate(Settings.Default.TemplateLibraryName, "QuoteSlip.docx");
        }

        private void clbQuestions_Validating(object sender, CancelEventArgs e)
        {
        ...
        private void clbQuestions_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            //else
            //{
            //    var checkedItemIndex = clbQuestions.CheckedIndices[0];
            //    clbQuestions.ItemCheck -= clbQuestions_ItemCheck;
        ...
        private void ColorFindMatches(DrawEventArgs e)
        {
            if (!string.IsNullOrEmpty(txtFindPolicyClass.Text))
            {
                ...
                {
                    e.TextColor = e.Text.ToUpper().Contains(txtFindPolicyClass.Text.ToUpper())
                        ? Color.Black
                        : Color.Gray;
                }
            }
        ...
        protected override List<IPolicyClass> LoadMinorPolicyTypes()
        {
            ...
            var spList = ListFactory.Create(Settings.Default.PreRenewalQuestionareMappingsListName);
        ...
        public void PopulateDocument(IBaseTemplate template, string previousCoverPageTitle, string previousLogoTitle,
            IDocument document)
        {
            ...
            tmpPresenter.CreateOrUpdateDocumentProperty(Constants.WordDocumentProperties.DocumentGeneratedDate,
                DateTime.Now.ToString(CultureInfo.CurrentUICulture));
        }

        private void PopulateQuestionFragments(IQuoteSlip template)
        {
            var c = clbQuestions.CheckedItems;
            ...
            foreach (var i in c)
            {
                ...
                var sh = LoadSchedule(Settings.Default.SharePointContextUrl, Settings.Default.PolicySchedulesListName,
                    t.Title);
                if (sh == null) continue;
                scheduleItems.Add(sh);
                var quoteSlipDoc = _document.OpenFile(Constants.CacheNames.GenerateQuoteSlip,
                    Settings.Default.TemplateQuoteSlip);
                //  var quoteSlipDoc = _document.OpenFile(Settings.Default.TemplateQuoteSlip);
                _wizardPresenter.CreateQuoteSlip(sh, quoteSlipDoc);

                PopulateDocument(template, lblCoverPageTitle.Text, lblLogoTitle.Text, quoteSlipDoc);
            }
        }
        ...
        private void QuoteSlipWizard_Load(object sender, EventArgs e)
        {
            ...
            if (Reload)
            {
                MessageBox.Show(@"You cannot make any changes to this quote slip through the wizard",
        ...
        private void LoadQuestions(TaskScheduler uiScheduler)
        {
            ...
            {
                Questions = LoadQuestionsFromSharePoint(Settings.Default.SharePointContextUrl,
                    Settings.Default.PolicySchedulesListName);
        ...
        private void clbQuestions_SelectedIndexChanged(object sender, EventArgs e)
        {
        }
    }
}