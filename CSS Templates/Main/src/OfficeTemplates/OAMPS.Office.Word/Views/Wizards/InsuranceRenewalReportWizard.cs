...
namespace OAMPS.Office.Word.Views.Wizards
{
    public partial class InsuranceRenewalReportWizard : BaseWizardForm
    {
        ...
        public InsuranceRenewalReportWizard(OfficeDocument document, Helpers.Enums.FormLoadType loadType)
        {
            ...
            //ShouldUpdateTemplate(Settings.Default.TemplateLibraryName, "Insurance Renewal Report.docx");
        }
            ...
            if (isChecked)
            {
                ...
                if (parent != null)
                {
                    ...
                    else
                    {
                        ...
                        {
                            foreach (var r in insurers.SelectedReccomendedInsurers)
                            {
                                node.Reccommended += r.Title + Constants.Seperators.Spaceseperator + r.Percent + "%" +
                                                     Constants.Seperators.Spaceseperator +
                                                     Constants.Seperators.Lineseperator;
                        ...
                        if (insurers.SelectedCurrentInsurers.Count > 1)
                        {
                            ...
                            {
                                node.Current += c.Title + Constants.Seperators.Spaceseperator + c.Percent + "%" +
        ...
        private void RegenerateFormLoad()
        {
            ...
            if (Cache.Contains(Constants.CacheNames.RegenerateTemplate))
            {
                ...
                _wizardPresenter.CreateOrUpdateDocumentProperty(Constants.WordDocumentProperties.Segment,
                    template.Segment.ToString());
                _wizardPresenter.CreateOrUpdateDocumentProperty(Constants.WordDocumentProperties.StatutoryInformation,
                    template.Statutory.ToString());
                _wizardPresenter.CreateOrUpdateDocumentProperty(Constants.WordDocumentProperties.Remuneration,
                    template.Remuneration.ToString());
                _wizardPresenter.CreateOrUpdateDocumentProperty(Constants.WordDocumentProperties.UFI,
                    template.PopulateUFI.ToString());
        ...
        private void ReloadPolicyClasses(IInsuranceRenewalReport v, bool reload)
        {
            ...
            for (var index = v.SelectedDocumentFragments.Count - 1; index >= 0; index--)
            {
                ...
                if (found == null)
                {
                    ...
                    //  found = MinorItems.FirstOrDefault(x => x.Title.StartsWith("other" + othercount, StringComparison.OrdinalIgnoreCase));
            ...
            foreach (var x in tvaPolicies.AllNodes)
            {
                ...
                if (dCheckBox != null)
                {
                    ...
                    if ((bool) value)
                    {
                        ...
                        var foundItems = MinorItems.Find(i => i.Title == policyClassValue.ToString());
                        var foundItem =
                            MinorItems.Find(
                        ...
                        item.Id = foundItem == null ? "o" : foundItem.Id;
        ...
        private void PopulateDocument()
        {
            ...
            _wizardPresenter.PopulateServiceLineAgrement(SelectedSegment, segmentDocuments,
                dtpPeriodOfInsuranceFrom.Value);
            ...
            _wizardPresenter.PopulateExecutiveSummary(_selectedRemuneration,
                Settings.Default.FragmentRREexSumFeeCommission,
            ...
            _wizardPresenter.DeleteContactTables(template);
        ...
        private void rdoRetailFSG_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload &&
                string.Equals(_storedStatutory, Enums.Statutory.None.ToString(), StringComparison.OrdinalIgnoreCase) &&
                rdoRetailFSG.Checked)
                if (ContinueWithSignificantChange(sender, true))
                    GenerateNewTemplate = true;
                else
                    rdoRetailFSG.Checked = false;
        }
        ...
        private void rdoWholesaleWithRetail_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload &&
                string.Equals(_storedStatutory, Enums.Statutory.None.ToString(), StringComparison.OrdinalIgnoreCase) &&
                rdoWholesaleWithRetail.Checked)
                if (ContinueWithSignificantChange(sender, true))
                    GenerateNewTemplate = true;
                else
                    rdoWholesaleWithRetail.Checked = false;
