...
namespace OAMPS.Office.Word.Views.Word
{
    public partial class TemplateUpdateChecker : Form
    {
        ...
        public bool CheckDocument(string shouldHide, string fragmentsUsedPropertyValue, string themes, string logos,
            string mainTemplate)
        {
        ...
        private bool CheckMainTemplate(string mainTemplate)
        {
            if (!string.IsNullOrEmpty(mainTemplate))
            {
                ...
                var itemList = ListFactory.Create(Settings.Default.MainTemplatesLibraryName);
                ...
                var usedCompateDate = new DateTime(logoUsedDateAsDate.Year, logoUsedDateAsDate.Month,
                    logoUsedDateAsDate.Day);
        ...
        private bool CheckTheme(string themes)
        {
            if (!string.IsNullOrEmpty(themes))
            {
                ...
                var name = themeInfo[0].Substring(themeInfo[0].LastIndexOf("/", StringComparison.Ordinal) + 1,
                    themeInfo[0].Length - themeInfo[0].LastIndexOf("/", StringComparison.Ordinal) - 1);
                var themeList = ListFactory.Create(Settings.Default.GraphicsPictureLibraryTitle);
        ...
        private bool CheckLogo(string logos)
        {
            if (logos != null)
            {
                if (!string.IsNullOrEmpty(logos))
                {
                    ...
                    var name = logoInfo[0].Substring(logoInfo[0].LastIndexOf("/", StringComparison.Ordinal) + 1,
                        logoInfo[0].Length - logoInfo[0].LastIndexOf("/", StringComparison.Ordinal) - 1);
                    var logosList = ListFactory.Create(Settings.Default.GraphicsPictureLibraryTitleLogos);
                    ...
                    if (usedCompateDate <= modifiedCompateDate)
                        //all done in a day buddy, i named it compate deal with it.
                    {
                        if (LatestChange < modifiedCompateDate)
                            LatestChange = modifiedCompateDate;
                        ...
                        dgUpdatedItems[2, (dgUpdatedItems.Rows.Count - 1)].Value =
                            logoUsedDateAsDate.ToString("dd/MM/yyyy");
                        dgUpdatedItems[3, (dgUpdatedItems.Rows.Count - 1)].Value =
                            spModifiedDateAsDate.ToString("dd/MM/yyyy");
