        ...
        public void PopulateBasisOfCover(List<IPolicyClass> fragements, string fragmentUrl)
        {
            ...
            for (var index = fragements.Count - 1; index >= 0; index--)
            {
                ...
                if (Document.MoveCursorToStartOfBookmark(bookmarkTemplate))
                {
                    ...
                    Document.RenameTable(Constants.WordTables.Liabilityrisksandexposures, Constants.WordTables.Liabilityrisksandexposures + f.Id + f.Order);
                    Document.RenameTable(Constants.WordTables.AssetRiskProtection, Constants.WordTables.AssetRiskProtection + f.Id + f.Order);
                    Document.RenameTable(Constants.WordTables.IncomeandotherFinancialExposures, Constants.WordTables.IncomeandotherFinancialExposures + f.Id + f.Order);
                    Document.RenameTable(Constants.WordTables.PolicyWording, Constants.WordTables.PolicyWording + f.Id + f.Order);


                    Document.RenameBookmark(Constants.WordBookmarks.SummaryOfCoverStart, Constants.WordBookmarks.SummaryOfCoverStart + f.Id + f.Order);
                    Document.RenameBookmark(Constants.WordBookmarks.SummaryOfCoverEnd, Constants.WordBookmarks.SummaryOfCoverEnd + f.Id + f.Order);
                }
                c++;
            }
                    ...
                    var p = new PolicyClass
                        {
                            ...
                            Title = Document.ReadContentControlValue(id + order + Constants.WordContentControls.ClassOfInsuranceTitle),
                            Id = id,
