...
namespace OAMPS.Office.BusinessLogic.Presenters.Wizards
{
    public class FactFinderWizardPresenter : BaseWizardPresenter
    {
        ...
        public void GenerateQuestionsOnly(List<IQuestionClass> list, string fragUrl)
        {
            ...
            foreach (var q in list)
            {
                if (q.Url != null)
                {
                    ...
                    if (d.HasBookmark(Constants.WordBookmarks.FactFinderStart))
                        d.RenameBookmark(Constants.WordBookmarks.FactFinderStart, Constants.WordBookmarks.FactFinderStart + q.Id);

                    if (d.HasBookmark(Constants.WordBookmarks.FactFinderEnd))
                        d.RenameBookmark(Constants.WordBookmarks.FactFinderEnd, Constants.WordBookmarks.FactFinderEnd + q.Id);
            ...
            Document.UpdateOrCreatePropertyValue(Constants.WordDocumentProperties.BuiltInTitle, Constants.TemplateNames.PreRenewalQuestionnaire);

            d.CloseInformationPanel(true);
            
        }

        public void PopulateImportantNotices(List<DocumentFragment> fragments, string importantNoticesUrl, string privacyStatementUrl, string fsgUrl, string termsOfEngagementUrl)
        {
