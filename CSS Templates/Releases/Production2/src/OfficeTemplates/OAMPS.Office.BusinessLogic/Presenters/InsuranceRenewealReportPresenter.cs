...
namespace OAMPS.Office.BusinessLogic.Presenters
{
    public class InsuranceRenewealReportPresenter : BasePresenter
    {
        ...
        {            
            Document.MoveCursorToStartOfBookmark(Constants.WordBookmarks.InsertClientProfile);
            ...
            if (rowCount > 0)
            {
                ...
                for (var i =0; i< fragements.Count;  i++)
                {
                    ...
                    for (var y = 1; y < columnCount; y++)
                    {
                        ...
                        {
                            Document.PopulateTableCell(position, y, f.RecommendedInsurer);
                        }
                        else
                        {
        ...
        public void PopulateBasisOfCover(List<IPolicyClass> fragements, string fragmentUrl)
        {
            ...
            for (int index = fragements.Count -1 ; index >= 0; index--)
            {
                ...
                Document.PopulateControl(f.CurrentInsurerId + "c" + order, f.CurrentInsurer);
                Document.PopulateControl(f.RecommendedInsurerId + "r" + order, f.RecommendedInsurer);                
                ...
                Document.UpdateOrCreatePropertyValue(Constants.WordDocumentProperties.IncludedPolicyTypes,
                                                     currentValue + ";" + f.Id + "_" + f.CurrentInsurerId + "c" + order + "_" + f.RecommendedInsurerId + "r" + order);
