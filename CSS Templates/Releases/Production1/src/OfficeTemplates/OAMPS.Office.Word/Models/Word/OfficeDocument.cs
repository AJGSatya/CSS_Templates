...
namespace OAMPS.Office.Word.Models.Word
{
    ...
    public class OfficeDocument : IDocument
    {
        ...
        public void ChangeDocumentImages(IList<string> imageAltText, IList<string> imageUrls)
        {
            ...
            for (var i = 1; i <= shapesCount; i++) //VSTO arrays start at 1
            {
                ...
                if (imageAltText.Contains(currentImage.AlternativeText))
                {
                    ...
                    var newImage =
                        _document.Sections[1].Footers[
                            WordOM.WdHeaderFooterIndex.wdHeaderFooterFirstPage].Shapes.AddPicture(
                                imageUrls[imageIndex], false,
                                true,
                                currentImage.Left,
                                currentImage.Top,
                                currentImage.Width,
        ...
        public void MoveCursorToStartOfBookmark(string name)
        {
        ...
        public void OpenFile(string serverRelativeUrl)
        {
            using (var clientContext = new ClientContext(Properties.Settings.Default.SharePointContextUrl))
            {
                var temp = System.IO.Path.GetTempFileName();
                ...
                _document.Application.Documents.Add(temp);
                System.IO.File.Delete(temp);
            }
        }

        public void OpenFile(string serverRelativeUrl, string cacheName)
        {
            using (var clientContext = new ClientContext(Properties.Settings.Default.SharePointContextUrl))
            {
                var temp = System.IO.Path.GetTempFileName();
...
// ReSharper restore UseIndexedProperty
        ...
        {            
            //_document.Application.ActiveDocument.Protect(WordOM.WdProtectionType.wdAllowOnlyReading);
            if (_document.Application.ActiveDocument.ProtectionType == WordOM.WdProtectionType.wdNoProtection)
            {
        ...
        {
            WordOM.WdProtectionType oldType = _document.Application.ActiveDocument.ProtectionType;
        ...
        {
            if(_document.ProtectionType != WordOM.WdProtectionType.wdNoProtection)
                return;
        ...
        public void SendEmail()
        {
            ...
            //email.Recipients.Add("dan.colarossi@oamps.com.au");
