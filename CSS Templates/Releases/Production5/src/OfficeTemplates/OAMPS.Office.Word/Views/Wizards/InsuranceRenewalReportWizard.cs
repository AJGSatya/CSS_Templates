...
// ReSharper restore LoopCanBeConvertedToQuery
            ...
            for (int index = v.SelectedDocumentFragments.Count - 1; index >= 0; index--)
            {
                ...
                foreach (var no in tvaPolicies.AllNodes)
                {
                    ...
                    if (String.Equals(no.Tag.ToString(), found.MajorClass, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            if (!String.Equals(cno.Tag.ToString(), found.Title, StringComparison.OrdinalIgnoreCase)) continue;
        ...
        private void rdoRetailFSG_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload && string.Equals(_storedStatutory, Enums.Statutory.None.ToString(), StringComparison.OrdinalIgnoreCase) && rdoRetailFSG.Checked)
        ...
        private void rdoWholesaleWithRetail_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload && string.Equals(_storedStatutory, Enums.Statutory.None.ToString(), StringComparison.OrdinalIgnoreCase) && rdoWholesaleWithRetail.Checked)
                if (ContinueWithSignificantChange(sender, true)) 
                    GenerateNewTemplate = true;
                else
                    rdoWholesaleWithRetail.Checked = false;
        ...
        private void rdoWholesale_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload && string.Equals(_storedStatutory, Enums.Statutory.None.ToString(), StringComparison.OrdinalIgnoreCase) && rdoWholesale.Checked)
                if (ContinueWithSignificantChange(sender, true))
                    GenerateNewTemplate = true;
                else
