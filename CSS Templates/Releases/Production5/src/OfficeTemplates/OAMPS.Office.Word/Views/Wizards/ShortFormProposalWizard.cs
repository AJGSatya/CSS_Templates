...
namespace OAMPS.Office.Word.Views.Wizards
{
    public partial class ShortFormProposalWizard : BaseWizardForm
    {
        ...
        protected static List<ISharePointListItem> DocumentTitles = null;
        ...
        {
            if (rdoSegment1.Checked == false && rdoSegment2.Checked == false && rdoSegment3.Checked == false && rdoSegment4.Checked == false &&
        ...
        private void LoadDocumentTitles(TaskScheduler uiScheduler)
        {
            ...
            {
                DocumentTitles = ((List<ISharePointListItem>)Cache.Get(Constants.CacheNames.ShortFormProposalDocumentTitles));
            }
            else
            {
            ...
            if (uiScheduler == null)
                ...
                Task.Factory.StartNew(()=>BindDocumentTitlesControl(DocumentTitles), CancellationToken.None, TaskCreationOptions.None, uiScheduler);

        }

        protected virtual List<ISharePointListItem> GetDocumentTitles()
        {
            var list = new SharePointList(Settings.Default.SharePointContextUrl, Settings.Default.ListProposalShortDocumentTitles, Constants.SharePointQueries.AllItemsSortBySortOrder);
        ...
        private InsuranceRenewalReport GenerateTempalteObject()
        {
            ...
            var template = new InsuranceRenewalReport
                {
                    DocumentTitle = cbDocumentTitle.Text,//BaseWizardPresenter.ReadDocumentProperty("Title"), //Constants.TemplateNames.InsuranceRenewalReport,
        ...
        private void PopulateDocument()
        {
            ...
            _wizardPresenter.PopulateBasisOfCover(_selectedDocumentFragments, Settings.Default.InsuranceProposalShortProgramSummary);
           ...
           // var image = GetServicePlanImage(SelectedSegment);
            ...
            {
                this.TopMost = false; //this is important, as there is a chance that outlook will prompt the users and if our wizard is forced to the top and outlook is minimised they will not see the outlook dialog and processing will hang.
                Task.Factory.StartNew(() => SendSeg4or5Email(template)).ContinueWith((task =>
                {
                    if (task.IsFaulted) OnError(task.Exception);
                }));
            }
        ...
        private string GetServicePlanImage(Enums.Segment segment)
        {
            ...
            var sharePointPictureLibry = new SharePointPictureLibrary(contextUrl, listTitle, false, String.Format(Constants.SharePointQueries.GetItemByTitleQuery,segment));
        ...
        private string GetServicePlanImageDoument(Enums.Segment segment)
        {
            ...
            else if (segment == Enums.Segment.Two)
                return Settings.Default.Segment2Graphic;
            else if (segment == Enums.Segment.Three)
                return Settings.Default.Segment3Graphic;
            else if (segment == Enums.Segment.Four)
                return Settings.Default.Segment4Graphic;
            else if (segment == Enums.Segment.Five)
        ...
        {
            var opdProfileDialog = new OpenFileDialog {InitialDirectory = _defaultPath, Filter = @"profiles (*.docx)|*.docx", FilterIndex = 1, RestoreDirectory = true};
        ...
        private void rdoSegment1_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (Reload && string.Equals(_storedSegment, Enums.Segment.One.ToString(), StringComparison.OrdinalIgnoreCase))
            {
