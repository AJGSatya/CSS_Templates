...
namespace OAMPS.Office.Word
{
    ...
    public class Ribbon : IRibbonExtensibility
    {
        ...
        public bool btnUpdateFields_GetEnabled(IRibbonControl control)
        {
            ...
            return (String.Equals(templateName, Constants.TemplateNames.InsuranceRenewalReport) || String.Equals(templateName, Constants.TemplateNames.ShortFormProposal));
        }

        public bool btnFactFinderButton_GetEnabled(IRibbonControl control)
        {
            dynamic templateName = ((DocumentProperties) (Globals.ThisAddIn.Application.ActiveDocument.BuiltInDocumentProperties))[WordOM.WdBuiltInProperty.wdPropertyTitle].Value.ToString();
            return String.Equals(templateName, Constants.TemplateNames.PreRenewalQuestionnaire) || String.Equals(templateName, Constants.TemplateNames.BlankFragmentBaseOlly);
        }
        ...
        public void btnConvertToManual_Click(IRibbonControl control)
        {
            ...
            foreach (var bookmark in allBookmarkedSections)
            {
                ...
                if (reportDoc.HasBookmark(Constants.WordBookmarks.SummaryOfCoverStart + id) && reportDoc.HasBookmark(Constants.WordBookmarks.SummaryOfCoverEnd + id))
                {
            ...
            if (reportDoc.HasBookmark(Constants.WordBookmarks.InsertClientProfile) && reportDoc.HasBookmark(Constants.WordBookmarks.InsertClientProfileEnd))
            {
