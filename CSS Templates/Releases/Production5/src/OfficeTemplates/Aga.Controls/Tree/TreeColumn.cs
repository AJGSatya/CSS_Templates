...
namespace Aga.Controls.Tree
{
    ...
    public class TreeColumn : Component
    {
        ...
        {
            var innerBounds = new Rectangle(bounds.X + HeaderLeftMargin, bounds.Y,
        ...
        private void DrawSortMark(Graphics gr, Rectangle bounds, int x)
        {
            ...
            {
                var points = new[] {new Point(x, y), new Point(x + SortMarkSize.Width, y), new Point(x + w2, y + SortMarkSize.Height)};
                gr.FillPolygon(SystemBrushes.ControlDark, points);
            }
            else if (SortOrder == SortOrder.Descending)
            {
                var points = new[] {new Point(x - 1, y + SortMarkSize.Height), new Point(x + SortMarkSize.Width, y + SortMarkSize.Height), new Point(x + w2, y - 1)};
                gr.FillPolygon(SystemBrushes.ControlDark, points);
            }
        }
